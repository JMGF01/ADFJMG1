...
import edu.adf.adfjmg1.databinding.ActivityInsertarClientesFirebaseBinding

const val URL_REAL_TIME_DATABASE = "https://projectone-78b7d-default-rtdb.europe-west1.firebasedatabase.app/"
...
class InsertarClientesFirebaseActivity : AppCompatActivity() {
    ...
    {
        val cliente = Cliente(binding.edadCliente.text.toString().toLong(),
        ...
        this.databaseReference.child("clientes").child(cliente.clave).setValue(cliente)
            .addOnCompleteListener{
                tarea -> Toast.makeText(this, "CLIENTE INSERTADO", Toast.LENGTH_LONG).show()
                    ...
                    Log.d(Constantes.ETIQUETA_LOG, "Error al insertar ", it)
            }
    }
    ...
    fun mostrarClientesFB(view: View)
    {
        ...
        this.databaseReference.child("clientes").get().addOnSuccessListener { datos ->
            ...
            Log.d(Constantes.ETIQUETA_LOG, "Hay $ncliens clientes FB")
            var cliente: Cliente
            ...
            entradas.forEach { (claveId, valor) ->
                ...
                if (listaClientes.size == ncliens) {
                    listaClientes.forEachIndexed { n, c ->
                        Log.d(
                            Constantes.ETIQUETA_LOG,
                            "Cliente $n = $c.toString()"
                        )
                    }
                }
            }
    ...
    fun borrarUltimoPorClave (view: View) {
        ...
        if (listaClientes.size > 0) {
            ...
            claveUltimo.let {
                ...
                val refCli = FirebaseDatabase.getInstance(URL_REAL_TIME_DATABASE)
                    .getReference("clientes/$claveUltimo")
            ...
            Toast.makeText(
                this,
                "Sin clientes que borrar/n Clique mostrar primero",
                Toast.LENGTH_LONG
            ).show()
        }
    }
...
//    fun borrarPorNombre (nombre:String){  // no funciona
//
  ...
  "rules": {
    ...
    fun borrarUltimoPorNombre(view: View) {
        ...
        if (listaClientes.size > 0) {
            ...
            Log.d(Constantes.ETIQUETA_LOG, "Quedan ${listaClientes.size} clientes")
        } else {
            Log.d(Constantes.ETIQUETA_LOG, "Sin clientes que borrar")
